...
public partial class RougeGameManager
{
    ...
    private SkillUpdateContext CreateSkillContext(float dt)
    {
        ...
        bool hasMouseGroundPoint = PlayerSkillMath.TryGetMouseGroundPoint(gameplayCamera, new Vector3(pointerPosition.x, pointerPosition.y, 0f), renderHeight, out Vector3 mouseGroundPoint);

        return new SkillUpdateContext(dt, playerPos, aimDir, hasMouseGroundPoint, mouseGroundPoint, renderHeight, arenaHalfExtent);
    }

    private bool IsSkillEnabled(PlayerSkillType type)
    {
        SkillPresentationConfig presentation = GetPresentationConfig(type);
        return presentation == null || presentation.Enabled;
    }

    private bool IsPlayerContactEnabled()
    {
        PlayerContactSkillConfig playerContact = skillConfig != null ? skillConfig.PlayerContact : null;
        return playerContact == null || playerContact.Presentation == null || playerContact.Presentation.Enabled;
    }

    private void ApplyPlayerContactSkillConfigValues()
    {
        PlayerContactSkillConfig playerContact = skillConfig.PlayerContact;
        playerContactDamage = Mathf.Max(0f, playerContact.GetValue(playerContact.PlayerDamage, currentLevel));
        playerHitInvincibilityDuration = Mathf.Max(0f, playerContact.GetValue(playerContact.InvincibilityDuration, currentLevel));
        playerContactPadding = Mathf.Max(0f, playerContact.GetValue(playerContact.ContactPadding, currentLevel));
        playerHitRepulseRadius = Mathf.Max(0f, playerContact.GetValue(playerContact.RepulseRadius, currentLevel));
        playerHitRepulseForce = playerContact.GetValue(playerContact.RepulseForce, currentLevel);
        playerHitRepulseLift = playerContact.GetValue(playerContact.RepulseLift, currentLevel);
        _playerContactRepulseDamage = Mathf.Max(0f, playerContact.GetValue(playerContact.RepulseDamage, currentLevel));
        _playerContactRingDuration = Mathf.Max(0f, playerContact.GetValue(playerContact.RingDuration, currentLevel));
        _playerContactDefeatEnemyOnContact = playerContact.DefeatEnemyOnContact;
    ...
    private void ApplySkillEffects(ref RougeSkillArea area, ResolvedSkillHitEffectConfig effects)
    {
        ...
        area.EffectKnockbackCenter = (int)effects.KnockbackCenter;
    ...
    private void ApplyPendingPlayerContactSkill()
    {
        ...
        if (!IsPlayerContactEnabled())
        {
            return;
        }

        bool hasContactArea = playerHitRepulseRadius > 0f &&
            (_playerContactRepulseDamage > 0f ||
            Mathf.Abs(playerHitRepulseForce) > 0.0001f ||
            Mathf.Abs(playerHitRepulseLift) > 0.0001f ||
            _playerContactEffects.Tags != SkillHitEffectTag.None);
        if (!hasContactArea)
        {
            return;
        }
    ...
    private void UpdateLeapSmashSkill(SkillUpdateContext context)
    {
        ...
        float leapLandingPullForce = 0f;
        float leapLandingVerticalForce = 0f;
    ...
    private void UpdateLightPillarSkill(SkillUpdateContext context)
    {
        ...
        float lightPillarPullForce = 0f;
        float lightPillarVerticalForce = 0f;
    ...
    private void UpdateBombSkill(SkillUpdateContext context)
    {
        ...
        float bombPullForce = 0f;
        float bombVerticalForce = 0f;
    ...
    {
        if (!IsSkillEnabled(PlayerSkillType.OrbitBall))
        {
            ClearOrbitVisuals();
            return;
        }
        ...
        float orbitPullForce = 0f;
        float orbitVerticalForce = 0f;
    ...
    private void UpdateShockwaveSkill(SkillUpdateContext context)
    {
        ...
        float shockwavePullForce = 0f;
        float shockwaveVerticalForce = 0f;
    ...
    private void UpdateMeteorSkill(SkillUpdateContext context)
    {
        ...
        float meteorPullForce = 0f;
        float meteorVerticalForce = 0f;
    ...
    private void UpdateIceZoneSkill(SkillUpdateContext context)
    {
        ...
        float iceBurstPullForce = 0f;
        float iceBurstVerticalForce = 0f;
    ...
    {
        return new Vector3(2.9f, 0.36f, 6.3f);
    }

    private static float GetSkateboardYaw(float2 direction)
    {
        float2 resolved = math.normalizesafe(direction, new float2(0f, 1f));
        return Mathf.Atan2(resolved.x, resolved.y) * Mathf.Rad2Deg;
    }

    private static float2 RotateSkateboardDirectionTowards(float2 currentDirection, float2 targetDirection, float maxTurnDegrees)
    {
        float2 current = math.normalizesafe(currentDirection, new float2(0f, 1f));
        float2 target = math.normalizesafe(targetDirection, current);
        float currentYaw = GetSkateboardYaw(current);
        float targetYaw = GetSkateboardYaw(target);
        float nextYaw = Mathf.MoveTowardsAngle(currentYaw, targetYaw, math.max(0f, maxTurnDegrees));
        float nextYawRadians = nextYaw * Mathf.Deg2Rad;
        return new float2(math.sin(nextYawRadians), math.cos(nextYawRadians));
    }

    private Vector3 ClampSkateboardWorldPosition(Vector3 position)
    {
        position.x = Mathf.Clamp(position.x, -arenaHalfExtent + 1f, arenaHalfExtent - 1f);
        position.z = Mathf.Clamp(position.z, -arenaHalfExtent + 1f, arenaHalfExtent - 1f);
        return position;
    }

    private float2 ResolveSkateboardDirection(SkillUpdateContext context)
    {
        ...
        if (math.lengthsq(_skateMoveDirection) > 0.0001f)
        {
            return _skateMoveDirection;
        }

        if (player != null)
        {
            Vector3 planarVelocity = player.Velocity;
            Vector2 velocityDir = new Vector2(planarVelocity.x, planarVelocity.z);
            if (velocityDir.sqrMagnitude > 0.0001f)
            {
                return velocityDir.normalized;
            }
        }

        return math.normalizesafe(context.AimDirection, new float2(0f, 1f));
    }

    private void AddSkateboardContactArea(float2 boardPosition, float radius, float damagePerSecond, ResolvedSkillHitEffectConfig effects)
    {
        if (radius <= 0f || (damagePerSecond <= 0f && effects.Tags == SkillHitEffectTag.None))
        {
            return;
        }
        ...
        TryAddSkillArea(area, effects);
    }

    private void BeginSkateboardFinale(Vector3 startPosition, float2 direction, float travelDistance, float finaleDuration)
    {
        _skatePhase = 5;
        _skatePhaseTimer = finaleDuration;
        _skateSlamFired = false;
        _skatePendingEnd = true;
        _skateActionDirection = math.normalizesafe(direction, math.normalizesafe(_skateMoveDirection, new float2(0f, 1f)));
        _skateFinaleStart = ClampSkateboardWorldPosition(startPosition);
        Vector3 forward = new Vector3(_skateActionDirection.x, 0f, _skateActionDirection.y);
        _skateFinaleEnd = ClampSkateboardWorldPosition(_skateFinaleStart + forward * travelDistance);
    ...
    {
        float invT = 1f - t;
        return invT * invT * a + 2f * invT * t * b + t * t * c;
    }

    private void EvaluateSkateboardTrickPose(
        int variant,
        ...
        out Quaternion boardRotation)
    {
        ...
        float jumpArc = math.sin(t * math.PI) * jumpHeight * 0.8f;
        float yaw = GetSkateboardYaw(_skateActionDirection);

        playerPos = Vector3.Lerp(_skateTrickOrigin, _skateTrickEnd, moveT);
        playerPos.y = renderHeight + jumpArc;
        playerForward = forward;
        boardPos = playerPos;
        boardRotation = Quaternion.Euler(0f, yaw, 0f);

        switch (variant)
        {
            case 5:
            {
                float flourish = math.sin(t * math.PI);
                float handSpin = t * 360f;
                float attachT = math.saturate((t - 0.68f) / 0.32f);
                float handSpinRad = handSpin * Mathf.Deg2Rad;
                Vector3 handCenter = playerPos + Vector3.up * (0.95f + flourish * 0.2f) + forward * 0.15f;
                Vector3 windmillPos = handCenter + right * (math.cos(handSpinRad) * 1.15f) + Vector3.up * (math.sin(handSpinRad) * 0.75f);
                Vector3 tuckPos = playerPos - forward * 0.2f;
                tuckPos.y = renderHeight + 0.18f + jumpArc * 0.3f;
                playerForward = (Quaternion.Euler(0f, t * 420f, 0f) * forward).normalized;
                boardPos = Vector3.Lerp(windmillPos, tuckPos, attachT * attachT * (3f - 2f * attachT));
                boardRotation = Quaternion.LookRotation(playerForward, Vector3.up) * Quaternion.Euler(handSpin, 0f, Mathf.Lerp(-110f, 0f, attachT));
                break;
            }
            case 4:
            {
                float burst = math.sin(t * math.PI);
                float surge = math.sin(t * math.PI * 2f);
                playerPos += forward * (burst * 0.95f) + right * (surge * 1.1f);
                playerForward = (Quaternion.Euler(0f, t * 300f, 0f) * Vector3.Slerp(forward, (forward + right * 0.85f).normalized, 0.5f + 0.5f * burst)).normalized;
                boardPos = playerPos + forward * 0.55f + right * (surge * 0.85f);
                boardPos.y = renderHeight + 0.22f + jumpArc * 0.68f + burst * 0.3f;
                boardRotation = Quaternion.Euler(35f + burst * 85f, yaw + t * 1320f, -140f * burst);
                break;
            }
            case 3:
            {
                float flare = math.sin(t * math.PI);
                float scissors = math.sin(t * math.PI * 3f);
                playerPos += right * (flare * 1.7f);
                playerForward = Vector3.Slerp(forward, (forward - right * 0.7f).normalized, flare * 0.72f);
                boardPos = playerPos + forward * 0.2f - right * (flare * 0.45f);
                boardPos.y = renderHeight + 0.2f + jumpArc * 0.76f + math.abs(scissors) * 0.18f;
                boardRotation = Quaternion.Euler(70f * flare, yaw + t * 1440f, 150f * scissors);
                break;
            }
            case 1:
            {
                float sideArc = math.sin(t * math.PI);
                float weave = math.sin(t * math.PI * 2f);
                playerPos += right * (sideArc * 1.35f);
                playerForward = Vector3.Slerp(forward, (forward + right * 0.6f).normalized, sideArc * 0.65f);
                boardPos = playerPos - forward * 0.3f + right * (weave * 0.65f);
                boardPos.y = renderHeight + 0.18f + jumpArc * 0.72f + sideArc * 0.25f;
                boardRotation = Quaternion.Euler(-15f + sideArc * 85f, yaw + t * 900f, -105f * sideArc);
                break;
            }
            case 2:
            {
                float bodySpin = math.sin(t * math.PI) * 0.9f;
                float weave = math.sin(t * math.PI * 2f);
                playerPos += right * (weave * 0.9f);
                playerForward = (Quaternion.Euler(0f, weave * 32f, 0f) * forward).normalized;
                boardPos = playerPos + forward * 0.4f + right * (weave * 0.55f);
                boardPos.y = renderHeight + 0.16f + jumpArc * 0.52f + bodySpin * 0.24f;
                boardRotation = Quaternion.Euler(22f * bodySpin, yaw + t * 1080f, 28f * weave);
                break;
            }
            default:
            {
                boardPos = playerPos - forward * 0.85f;
                boardPos.y = renderHeight + 0.14f + jumpArc * 0.55f;
                boardRotation = Quaternion.Euler(0f, yaw + t * 540f, 55f * math.sin(t * math.PI));
                break;
            }
        }

        boardPos = ClampSkateboardWorldPosition(boardPos);
        boardPos.y = Mathf.Max(boardPos.y, renderHeight + boardScale.y * 0.35f);
        playerForward = playerForward.sqrMagnitude > 0.001f ? playerForward.normalized : forward;
    }

    private static Quaternion EvaluateSkateboardFinaleRotation(Vector3 finaleForward, float normalizedTime)
    {
        float t = math.saturate(normalizedTime);
        float spinDegrees = t * 1260f;
        Vector3 noseDownForward = Vector3.Slerp(finaleForward, (finaleForward - Vector3.up).normalized, math.saturate((t - 0.72f) / 0.28f));
        Quaternion baseRotation = Quaternion.LookRotation(noseDownForward, Vector3.up);
        return baseRotation * Quaternion.Euler(spinDegrees, 0f, 0f);
    }

    private static int GetRandomSkateboardTrickVariant(int previousVariant)
    {
        const int trickVariantCount = 6;
        int nextVariant = UnityEngine.Random.Range(0, trickVariantCount);
        if (trickVariantCount > 1 && nextVariant == previousVariant)
        {
            nextVariant = (nextVariant + 1) % trickVariantCount;
        }
    ...
    private void UpdateSkateboardSkill(SkillUpdateContext context)
    {
        ...
        float rideSpeed   = cfg.GetValue(cfg.RideSpeed,        lvl) * 2f;
        float rideHitRadius = cfg.GetValue(cfg.RideContactRadius, lvl) * 2f;
        ...
        float launchForwardDistance = math.max(rideSpeed * math.max(initJumpDur, 0.08f) * 0.45f, 1.25f);
        float trickForwardDistance = math.max(rideSpeed * trickDur * 0.75f, 3.25f);
        float finaleForwardDistance = math.max(rideSpeed * finaleDur * 0.8f, 5.75f);
        float finaleBoardForwardOffset = math.max(boardScale.z * 0.42f, 1.6f);
            ...
            && TryStartSkillActivation(PlayerSkillType.Skateboard))
        {
            ...
            _skateSlamFired     = false;
            _skateOriginPos     = ClampSkateboardWorldPosition(player.transform.position);
            _skateBoardPos      = context.PlayerPosition;
            _skateMoveDirection = ResolveSkateboardDirection(context);
            _skateActionDirection = _skateMoveDirection;
            Vector3 launchForward = new Vector3(_skateMoveDirection.x, 0f, _skateMoveDirection.y);
            _skateLaunchEnd     = ClampSkateboardWorldPosition(_skateOriginPos + launchForward * launchForwardDistance);
            _skateLaunchEnd.y   = renderHeight;
            _skateBoardVelocity = (Vector2)_skateMoveDirection * (rideSpeed * 0.35f);
            _skateBoardRotYaw   = GetSkateboardYaw(_skateMoveDirection);
            _skateTrickOrigin   = _skateOriginPos;
            _skateTrickEnd      = _skateOriginPos;
            _skateTrickVariant  = 2;
        ...
        if (_skatePhase == 1)
        {
            ...
            Vector3 pos  = Vector3.Lerp(_skateOriginPos, _skateLaunchEnd, moveT);
            pos.y = renderHeight + yh;
            player.transform.position = pos;
            player.SuppressMovement   = true;
            player.transform.forward = new Vector3(_skateMoveDirection.x, 0f, _skateMoveDirection.y);
            _invincibilityTimer       = math.max(_invincibilityTimer, dt + 0.05f);

            // 滑板在玩家脚下旋转
            _skateBoardRotYaw += 360f * dt * 2.5f;
            _skateBoardVisual.SetActive(true);
            _skateBoardVisual.transform.position   = new Vector3(pos.x, renderHeight + 0.12f + yh * 0.18f, pos.z);
        ...
        if (_skatePhase == 2)
        {
            player.transform.position = new Vector3(_skateLaunchEnd.x, renderHeight, _skateLaunchEnd.z);
            player.SuppressMovement   = true;
            player.transform.forward = new Vector3(_skateMoveDirection.x, 0f, _skateMoveDirection.y);
            _invincibilityTimer       = math.max(_invincibilityTimer, dt + 0.05f);
            ...
            _skateBoardVisual.SetActive(true);
            _skateBoardVisual.transform.position   = new Vector3(_skateLaunchEnd.x, renderHeight + 0.12f, _skateLaunchEnd.z);
            _skateBoardVisual.transform.rotation   = Quaternion.Euler(0f, _skateBoardRotYaw, 0f);
            _skateBoardVisual.transform.localScale  = boardScale;
            ...
            if (_skatePhaseTimer <= 0f)
            {
                _skatePhase      = 3;
                _skatePhaseTimer = 0f;
                _skateBoardVelocity = (Vector2)_skateMoveDirection * (rideSpeed * 0.55f);
            }
            return;
        }
        ...
        if (_skatePhase == 3)
        {
            ...
            float2 moveDirection = ResolveSkateboardDirection(context);
            if (math.lengthsq(moveDirection) > 0.0001f)
            {
                _skateMoveDirection = math.normalizesafe(moveDirection, new float2(0f, 1f));
            }
            ...
            if (pressed && _skatePhase == 3)
            {
                ...
                _skateTrickVariant = GetRandomSkateboardTrickVariant(_skateTrickVariant);
                _skateTrickOrigin = ClampSkateboardWorldPosition(player.transform.position);
                float2 boardVelocityDir = math.normalizesafe(new float2(_skateBoardVelocity.x, _skateBoardVelocity.y), _skateMoveDirection);
                _skateActionDirection = math.normalizesafe(boardVelocityDir, _skateMoveDirection);
                Vector3 trickForward = new Vector3(_skateActionDirection.x, 0f, _skateActionDirection.y);
                _skateTrickEnd = ClampSkateboardWorldPosition(_skateTrickOrigin + trickForward * trickForwardDistance);
                _skateTrickEnd.y = renderHeight;
                _invincibilityTimer = math.max(_invincibilityTimer, trickDur + finaleDur + 0.15f);
                return;
            }
            ...
            {
                float2 boardVelocityDir = math.normalizesafe(new float2(_skateBoardVelocity.x, _skateBoardVelocity.y), _skateMoveDirection);
                BeginSkateboardFinale(player.transform.position, boardVelocityDir, finaleForwardDistance, finaleDur);
                return;
            }

            // 朝 WASD 方向推进，松开按键后保留最后的非零方向
            float2 desiredDir  = math.normalizesafe(_skateMoveDirection, new float2(0f, 1f));
            float2 currentDir = math.normalizesafe(new float2(_skateBoardVelocity.x, _skateBoardVelocity.y), desiredDir);
            float2 steeredDir = RotateSkateboardDirectionTowards(currentDir, desiredDir, rideTurnRate * dt);
            _skateBoardVelocity = new Vector2(steeredDir.x, steeredDir.y) * rideSpeed;

            Vector3 playerPos  = ClampSkateboardWorldPosition(player.transform.position);
            playerPos.x       += _skateBoardVelocity.x * dt;
            playerPos.z       += _skateBoardVelocity.y * dt;
            playerPos.y        = renderHeight;
            playerPos = ClampSkateboardWorldPosition(playerPos);
            player.transform.position = playerPos;
            player.SuppressMovement   = true;
            player.transform.forward = new Vector3(steeredDir.x, 0f, steeredDir.y);

            // 滑板朝移动方向
            if (_skateBoardVelocity.sqrMagnitude > 0.01f)
            {
                _skateBoardRotYaw = GetSkateboardYaw(new float2(_skateBoardVelocity.x, _skateBoardVelocity.y));
            }
            ...
            _skateBoardVisual.transform.rotation   = Quaternion.Euler(0f, _skateBoardRotYaw, 0f);
            _skateBoardVisual.transform.localScale  = boardScale;
            AddSkateboardContactArea(_skateBoardPos, rideHitRadius, rideHitDps, rideFx);
            return;
        }
        ...
        if (_skatePhase == 4)
        {
            ...
            EvaluateSkateboardTrickPose(_skateTrickVariant, t, jumpH, boardScale, out Vector3 pos, out Vector3 trickFacing, out Vector3 boardPos, out Quaternion boardRotation);
            player.transform.position = pos;
            player.SuppressMovement   = true;
            player.transform.forward = trickFacing;
            _invincibilityTimer       = math.max(_invincibilityTimer, dt + 0.05f);

            _skateBoardVisual.SetActive(true);
            _skateBoardVisual.transform.position  = boardPos;
            _skateBoardVisual.transform.rotation  = boardRotation;
            _skateBoardVisual.transform.localScale = boardScale;
            _skateBoardPos = new float2(boardPos.x, boardPos.z);
            _skateBoardRotYaw = boardRotation.eulerAngles.y;
            AddSkateboardContactArea(_skateBoardPos, rideHitRadius * 0.95f, rideHitDps, rideFx);

            if (_skatePhaseTimer <= 0f)
            {
                Vector3 trickEndPos = ClampSkateboardWorldPosition(_skateTrickEnd);
                trickEndPos.y = renderHeight;
                player.transform.position = trickEndPos;
                _skateBoardVelocity = new Vector2(_skateActionDirection.x, _skateActionDirection.y) * rideSpeed;
                _skateMoveDirection = _skateActionDirection;
                if (_skateRideTimer <= 0f)
                {
                    BeginSkateboardFinale(trickEndPos, _skateActionDirection, finaleForwardDistance, finaleDur);
                }
                else
                {
                    _skatePhase = 3;
        ...
        if (_skatePhase == 5)
        {
            ...
            float moveT = t * t * (3f - 2f * t);
            Vector3 finaleForward = new Vector3(_skateActionDirection.x, 0f, _skateActionDirection.y);
            Vector3 finaleRight = new Vector3(finaleForward.z, 0f, -finaleForward.x);

            // 玩家沿弧线前跳
            float jumpArc = math.sin(moveT * math.PI) * jumpH * 1.2f;
            Vector3 playerP = Vector3.Lerp(_skateFinaleStart, _skateFinaleEnd, moveT);
            playerP.y = renderHeight + jumpArc;
            player.transform.position = playerP;
            player.SuppressMovement   = true;
            player.transform.forward = (Quaternion.Euler(0f, moveT * 540f, 0f) * finaleForward).normalized;
            _invincibilityTimer       = math.max(_invincibilityTimer, dt + 0.05f);

            // 滑板以玩家为中心，在竖直平面做三圈半轮胎转，最后头朝地拍下
            float orbitDegrees = -90f + moveT * 1260f;
            float orbitRadians = orbitDegrees * Mathf.Deg2Rad;
            float orbitForwardRadius = math.max(boardScale.z * 0.52f, 1.5f);
            float orbitVerticalRadius = math.max(jumpH * 0.72f, 2.2f);
            float sideDrift = math.sin(moveT * math.PI * 2f) * boardScale.x * 0.12f;
            Vector3 boardPos = playerP
                + finaleForward * (math.cos(orbitRadians) * orbitForwardRadius)
                + Vector3.up * (math.sin(orbitRadians) * orbitVerticalRadius - 1f)
                + finaleRight * sideDrift;
            Vector3 boardEnd = _skateFinaleEnd + finaleForward * finaleBoardForwardOffset + Vector3.down;
            if (moveT >= 0.84f)
            {
                float slamT = math.saturate((moveT - 0.84f) / 0.16f);
                boardPos = Vector3.Lerp(boardPos, boardEnd, slamT * slamT * (3f - 2f * slamT));
            }
            Quaternion boardRot = EvaluateSkateboardFinaleRotation(finaleForward, moveT);
            _skateBoardVisual.SetActive(true);
            _skateBoardVisual.transform.position   = boardPos;
            _skateBoardVisual.transform.rotation   = boardRot;
            _skateBoardVisual.transform.localScale  = boardScale;
            _skateBoardPos = new float2(boardPos.x, boardPos.z);
            AddSkateboardContactArea(_skateBoardPos, rideHitRadius * 1.1f, rideHitDps * 1.15f, rideFx);

            // 收尾拍地只会在落点附近触发一次，动作过程中不会提前结束
            if (!_skateSlamFired && moveT >= 0.82f)
            {
            ...
            if (_skatePhaseTimer <= 0f)
            {
                ...
                if (player != null)
                {
                    ...
                    player.transform.forward = finaleForward;

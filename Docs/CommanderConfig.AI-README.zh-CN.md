...
- 存钱 ↔ 铺塔，二者之和 50。
- 控场减益 ↔ 直接伤害，二者之和 50。
- 清群 ↔ 攻坚（单体、破甲、精英/首领），二者之和 50。

中性为每项 25。六项不可能同时拉满。具体公式见完整规范。

塔本身的能力不由角色配置猜测。`tower_defense_balance.json` 中每种塔通过
`aiRoleProfile` 声明 0–1 的 `control`、`directDamage`、`singleTarget`、
`areaDamage`、`armorBreaking`；会改变职责的升级路线通过
`aiSpecializations` 声明升级后的同一组能力。角色雷达只对这些能力加权，
不要在 commander JSON 中复制塔能力。

当前塔池中，`control` 专指冰塔的减速/冻结体系，`singleTarget` 专指机枪与
激光体系（包括它们的破甲路线）。偏清群分支仍可保留较低的单体值，但其余
塔不应靠泛化伤害值计入这两项阵容保底。新增或重做塔时只调整上述能力配置，
不要在 AI 决策代码里按塔名追加例外。

...
namespace HighPerform.test.Scripts
{
    public class LiquidTest : MonoBehaviour
    {
        ...
        private readonly int _hashSize = 1048576;
        private readonly int _hashMask = 1048575;
        ...
        private GraphicsBuffer _posRadBuffer, _colorBuffer, _argsBuffer;
        ...
        private void Create()
        {
            ...
            _posRadBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, _currentParticleCount, 16);
        ...
        void PhysicsJob(float dt)
        {
            ...
            handle = new ClearGridJob { CellCounts = _cellCounts }.ScheduleBatch(_hashSize, 16384, handle);
    ...
    public unsafe struct CopyArrayJob : IJobParallelForBatch
    {
        ...
        public void Execute(int startIndex, int count)
        {
            ulong* srcPtr = (ulong*)Src.GetUnsafeReadOnlyPtr();
            ulong* dstPtr = (ulong*)Dst.GetUnsafePtr();
            int end = startIndex + count;
            int i = startIndex;
            for (; i <= end - 4; i += 4)
            {
        ...
        public void Execute(int startIndex, int count)
        {
            ...
            for (int i = startIndex; i < end; i++)
            {
                ...
                if (i == 0 || h != (int)(keysPtr[i - 1] >> 32))
                {
                    ...
                    int currentCount = 1;
                    while (i + currentCount < Keys.Length && (int)(keysPtr[i + currentCount] >> 32) == h)
                    {
                        currentCount++;
                    }

                    countsPtr[h] = currentCount;
                }
            }
        }
    }
